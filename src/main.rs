        ...
        Commands::Check { file, cloak } => {
            ...
            for key in result.keys {
                ...
                for err in key.errors.clone() {
                    ...
                    if cloak {
                        let trimmed_val = trim_quotes(&string_val);
...
"#,
                        ...
                        key.position.line,
                        key.position.column,
                        number_pad(key.position.line, 3).to_string().blue(),
                        error_squiggles.red()
                    );
...
fn number_pad(num: u32, min_length: usize) -> String {
    let num_str = num.to_string();

    let padding = min_length - num_str.len() - 1;
