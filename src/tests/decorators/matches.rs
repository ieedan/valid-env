...
fn string_fails() {
    ...
    let expected_error_message =
        format!("'{expected_value}' does not match '{pattern}'.");

    let content = format!("@matches(\"{pattern}\")\n{expected_key}=\"{expected_value}\"");
...
fn string_array_fails() {
    ...
    let pattern = "^\\d+$";
    let expected_key = "SOMETHING";
    let failed_value = "1234five6";
    let expected_value = vec![failed_value, "654321", "123456"];

    let expected_error_message =
        format!("'{failed_value}' does not match '{pattern}'.");
...
fn string_array_passes() {
    ...
    let pattern = "^\\d+$";
    let expected_key = "SOMETHING";
    let expected_value = vec!["123456", "654321", "123456"];
